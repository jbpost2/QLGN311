...
Conducting a quick comparison of these sample proportions by question
...
sum_glm <- function(data, response){ 
  ...
  ret <- c(substring(response, resp[[1]], resp[[1]]+attr(resp[[1]],'match.length')-1), round(c(fit$coefficients, summary(fit)$coefficients[2,4]), 4))
  names(ret) <- c("Response", "Intercept", "Beta1", "p-value")
  ret
}

bin_names <- c("HW6_Q_3_score_bin", "HW6_Q_4_score_bin", "HW6_Q_5_score_bin",
               "HW6_Q_6_score_bin", "HW6_Q_7_score_bin", "HW6_Q_8_score_bin",
               "HW6_Q_9_score_bin", "HW6_Q_10_score_bin", "HW6_Q_15_score_bin",
...
To further investigate the effect of the intervention we'll look at a break down comparing cohorts for correct/incorrect question status by:  

+ ST 311 status (not taken, currently in, previously taken)  
+ ST 311 GPA for those that have taken the course  
+ highest math course taken with indications for grade  
+ class level (senior, etc.)
+ race  
+ sex   
...
A logistic regression model will also be fit to take into account multiple explanatory variables at once.

We'll also consider just the students that missed at least one question from the earlier homework assignment.  These students are students that clearly have room for improvement.  
